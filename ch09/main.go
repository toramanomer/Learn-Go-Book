...
func (e EmptyField) Error() string {
	return fmt.Sprintf("field %q is empty", e.Name)
}
...
func processError(err error, emp Employee) string {
	var fieldErr EmptyField
	switch {
	case errors.Is(err, ErrInvalidID):
		return fmt.Sprintf("invalid ID: %s", emp.ID)
	case errors.As(err, &fieldErr):
		return fmt.Sprintf("empty field %q", fieldErr.Name)
	default:
...
func ValidateEmployee(e Employee) error {
	var errs []error
	if len(e.ID) == 0 {
		errs = append(errs, EmptyField{Name: "ID"})
	}
	if !validID.MatchString(e.ID) {
		errs = append(errs, ErrInvalidID)
	}
	if len(e.FirstName) == 0 {
		errs = append(errs, EmptyField{Name: "FirstName"})
	}
	if len(e.LastName) == 0 {
		errs = append(errs, EmptyField{Name: "LastName"})
	}
	if len(e.Title) == 0 {
		errs = append(errs, EmptyField{Name: "Title"})
	}
